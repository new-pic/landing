...
핑크/코랄 톤의 Newpic 앱 소개 랜딩 페이지입니다. Vite + React로 만들어졌고,
...
- 문구 수정/언어 추가는 `src/i18n.jsx`의 `MESSAGES`에서 하면 됩니다.
...
src/
  ...
  index.css              # 디자인 시스템 (색상 변수는 :root 참고)
  App.jsx
  components/
...
브랜드 컬러를 바꾸려면 `src/index.css` 맨 위 `:root`의 `--coral-*` 변수만 수정하세요.
